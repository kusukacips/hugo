...
image_files_to_try = [
    os.path.join("semangat", "folder", "boxing_cat.jpeg")
]
...
if st.session_state.cat_clicked:
    ...
    # Daftar gambar yang akan ditampilkan (pastikan file-nya benar dan ada di GitHub)
    gift_images = [
        "ayam.jpeg", "kopi.jpeg", "kucing.jpeg",
        "love.jpeg", "cipa.jpeg", "susu.png", "semangat.jpeg"
    ]

    # Dua kolom untuk layout
    cols = st.columns(2)

    # Loop untuk menampilkan setiap gambar
    for i, img_name in enumerate(gift_images):
        image_path = os.path.join("semangat", "folder", img_name)
        img_base64 = get_image_base64(image_path)

        with cols[i % 2]:  # Ganti-gantian kolom
            if img_base64:
                st.markdown(
                ...
                st.error(f"Could not load gift image: {image_path}")
